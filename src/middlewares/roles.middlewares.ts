...
export const hasPermission = (operation: ModelOperation, modelName: string)=> {
    return async (req: Request, res: Response, next: NextFunction) => {
        ...
        const rolePermissions = await RolePermission.findAll({ where: { roleId: req.auth.roleId } })
        const permissions = await Permission.count({
            where: {
                id: {[Op.in]: rolePermissions.map(rolePermission=> rolePermission.permissionId)},
                operations: {[Op.contains]: [operation]},
                modelName: modelName
            }
        })
        
        if(permissions < 1) return res.status(403).send({
            success: false,
            message: "Forbidden",
            status: 403
        })
